...
# Forge Development Standards — Python

## Naming Conventions

- Modules & functions: snake_case (`get_user_by_id`, `is_active`)
...
- List/dict/set comprehensions where readable; avoid deeply nested ones
- Virtual environments for dependency isolation

## Type Safety

Type hints on all public functions. Use `mypy` or `pyright` for static checking. Avoid `Any` — use `Union`, `Optional`, or generics.

## Module Structure

Package per domain with `__init__.py`. Keep modules focused. Use relative imports within packages.

## Test Conventions

- Test file pattern: `foo.py` -> `test_foo.py` or `tests/test_foo.py`
- Framework: pytest
